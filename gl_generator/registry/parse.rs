...
use xml::{
  attribute::OwnedAttribute, reader::XmlEvent, EventReader as XmlEventReader,
};
...
fn merge_map(
  a: &mut BTreeMap<String, Vec<String>>,
...
trait Parse: Sized + Iterator<Item = ParseEvent> {
  fn parse(mut self, filter: &Filter, require_feature: bool) -> Registry {
    ...
    while let Some(event) = self.next() {
      match event {
        // ignores
        ParseEvent::Text(_) => (),
        ParseEvent::Start(ref name, _) if name == "comment" => {
          self.skip_to_end("comment")
        }
        ParseEvent::Start(ref name, _) if name == "types" => {
          self.skip_to_end("types")
        }
        ...
        ParseEvent::Start(ref name, ref attributes) if name == "enums" => {
          ...
          if let Some(group) =
            enums_group.and_then(|name| groups.get_mut(&name))
          {
  ...
  fn consume_enums(&mut self, api: Api) -> Vec<Enum> {
    ...
    loop {
      match self.next().unwrap() {
        // ignores
        ParseEvent::Text(_) => {}
        ParseEvent::Start(ref name, _) if name == "unused" => {
          self.skip_to_end("unused")
        }
  ...
  fn consume_enum(&mut self, api: Api, attributes: &[Attribute]) -> Enum {
    let ident =
      trim_enum_prefix(&get_attribute(&attributes, "name").unwrap(), api);
  ...
  fn consume_cmds(
    &mut self,
    api: Api,
  ) -> (Vec<Cmd>, BTreeMap<String, Vec<String>>) {
    let mut cmds = Vec::new();
...
impl FromXml for Feature {
  fn convert<P: Parse>(parser: &mut P, a: &[Attribute]) -> Feature {
    ...
    let (requires, removes) =
      parser.consume_two("require", "remove", "feature");
...
impl FromXml for Extension {
  fn convert<P: Parse>(parser: &mut P, a: &[Attribute]) -> Extension {
    ...
    let supported = get_attribute(a, "supported")
      ...
      .filter_map(|api| {
        api_from_str(api)
          .unwrap_or_else(|()| panic!("unsupported API `{}`", api))
      })
      .collect::<Vec<_>>();
...
mod tests {
  ...
  mod make_enum {
    ...
    fn test_value_str() {
      let e =
        parse::make_enum("FOO".to_string(), None, "\"hi\"".to_string(), None);
  ...
  mod make_egl_enum {
    ...
    fn test_negative_value() {
      let e =
        parse::make_egl_enum("FOO".to_string(), None, "-1".to_string(), None);
